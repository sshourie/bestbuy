...
def Sanchita_feat(df_imputed_data):

    df_imputed_data.drop('Unnamed: 0', axis=1, inplace=True)
    df_imputed_data['CAST_SALES_DATE'] = df_imputed_data["SALES_DATE"].astype('datetime64[ns]')
    df_sort = df_imputed_data.sort_values(by=['Encoded_SKU_ID', 'CAST_SALES_DATE'])

    df_sort = df_imputed_data.sort_values(by=['Encoded_SKU_ID', 'CAST_SALES_DATE'])

    # Values corresponding to previous days

    df_sort['PREV_1_DAY_SALES'] = df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(1)
    df_sort['PREV_7_DAY_SALES'] = df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(7)
    df_sort['PREV_30_DAY_SALES'] = df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(30)
    df_sort['PREV_365_DAY_SALES'] = df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(365)
    # Total of previous periods

    df_sort['total_prev_7_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=7,
                                                                                                 min_periods=0).sum().reset_index(
        level=0, drop=True)
    df_sort['total_prev_30_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=30,
                                                                                                  min_periods=0).sum().reset_index(
        level=0, drop=True)
    df_sort['total_prev_91_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=91,
                                                                                                  min_periods=0).sum().reset_index(
        level=0, drop=True)
    df_sort['total_prev_365_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=365,
                                                                                                   min_periods=0).sum().reset_index(
        level=0, drop=True)
    # Total of previous periods

    df_sort['mean_prev_7_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=7,
                                                                                                min_periods=0).sum().reset_index(
        level=0, drop=True) / 7
    df_sort['mean_prev_30_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=30,
                                                                                                 min_periods=0).sum().reset_index(
        level=0, drop=True) / 30
    df_sort['mean_prev_91_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=91,
                                                                                                 min_periods=0).sum().reset_index(
        level=0, drop=True) / 91
    df_sort['mean_prev_365_days'] = df_sort.groupby('Encoded_SKU_ID')['PREV_1_DAY_SALES'].rolling(window=365,
                                                                                                  min_periods=0).sum().reset_index(
        level=0, drop=True) / 365
    # Sum of the week happening a year ago

    df_sort['year_ago_week_sum'] = df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(365) +
    df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(364) +
    df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(363) +
    df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(362) +
    df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(366) +
    df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(367) +
    df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(368)


    df_sort['year_ago_week_mean'] = (df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(365) +
                                     df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(364) +
                                     df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(363) +
                                     df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(362) +
                                     df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(366) +
                                     df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(367) +
                                     df_sort.groupby('Encoded_SKU_ID')['DAILY_UNITS'].shift(368)) / 7
    return df_sort
# df = pd.read_csv('data_imputed.csv')
orig_df = pd.read_csv('final_output_seas-3.csv')
# append validation data here
f1 = Sanchita_feat(orig_df)
f2 = full_feat(orig_df)